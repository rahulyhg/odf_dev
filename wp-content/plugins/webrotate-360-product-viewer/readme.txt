...
* ADDED: custom viewer cursors, pause viewer on load, zoom on single click, and more.

= 3.1.3 =
* ADDED: gallery integration in WooCommerce.
...
* FIXED: z-index was lowered across viewer elements to work better with WP themes.
* FIXED: updated viewer CSS for sharper image rendering in Chromium based browsers on Windows.
* FIXED: dummy license.lic was added to prevent 404 errors from slowing down loading of the 360 product viewer.
* FIXED: background color property now works for popup shortcodes.

= 3.1.2 =
* ADDED: WooCommerce v3.x support (single 360 product view; no gallery support yet).
* FIXED: WebRotate tab icon not showing under WooCommerce product in admin.
* FIXED: thin skin's SVG icons were showing odd artifacts on some video cards.
...
* ADDED: latest v3.6 viewer core (released version).

= 3.1.0 =
* ADDED: viewer API integration.
* ADDED: background color property.
* ADDED: minimum height property.
* ADDED: graphics path property for custom hotspots.
* ADDED: updated shortcodes to use data-imagerotator, eliminating script duplication and initialization issues.
* ADDED: latest v3.6 viewer core.
* ADDED: browserfs shortcode parameter to allow full browser window mode in wr360expand.
* FIXED: global script function to not duplicate on multiple posts.
* FIXED: reduced the number of global functions to one and only for popup shortcodes.
* FIXED: lightbox popup now correctly re-center upon browser resize via window maximize & minimize.
